...
function parseDateString(value) {
  var result = new Date();
  var regex1 = /\d\d\d\d-[01]\d-[0-3]\d/;
  var regex2 = /[0-3]\d-[01]\d-\d\d\d\d/;
  if (regex1.test(value) || regex2.test(value)) {
    value = value.split(/-/);
    result.setMonth(value[1] - 1);
    if (value[0].length === 4) {
      result.setFullYear(value[0]);
      result.setDate(value[2]);
    } else {
      result.setFullYear(value[2]);
      result.setDate(value[0]);
    }
    return result;
  }
...
function toDateString(value, format) {
  var regex1 = /YYYY-MM-DD/;
  var regex2 = /[MD][MD]-[DM][DM]-YYYY/;
  if (Date.parse(value) && (regex1.test(format) || regex2.test(format))) {
    var result;
    ...
    format = format.split(/-/);
    if (/YYYY/.test(format[0])) result = year + '-' + month + '-' + date;
    else if (/DD/.test(format[0])) result = date + '-' + month + '-' + year;
    else result = month + '-' + date + '-' + year;
    return result;
  }
...
function normalizeDuration(value) {
  var regex = /^[0-2]?\d[:h][0-5]?\d[:m][0-5]?\d[s]?$/;
  var result = null;
  if (regex.test(value)) {
    value = value.split(/[hms:]/);
    if (value.length === 4) value.pop();
...
function formatDurations(...values) {
  ...
  for (var i = 0; i < values.length; i++) {
    var value = normalizeDuration(values[i]);
    if (value !== null) {
      flag++;
      result = result + value + ', ';
    }
...
function generateLicenseBadge(licenseCode) {
  ...
  var finalURL;
  var code;
  if (typeof licenseCode === 'undefined' || licenseCode === null) {
    attribute = '>All Rights Reserved</a>';
    url = '<a href="https://choosealicense.com/no-permission/"';
  } else if (/CC-/.test(licenseCode.slice(0, 3))) {
    licenseCode = licenseCode.trim().toLowerCase();
    code = licenseCode.slice(3, licenseCode.length);

    switch (licenseCode) {
      case 'cc-by':
        attribute = 'alt="Creative Commons Attribution License"></a>';
        break;
      case 'cc-by-nc':
        attribute = 'alt="Creative Commons Attribution-NonCommercial License"></a>';
        break;
      case 'cc-by-sa':
        attribute = 'alt="Creative Commons Attribution-ShareAlike License"></a>';
        break;
      case 'cc-by-nd':
        attribute = 'alt="Creative Commons Attribution-NoDerivs License"></a>';
        break;
      case 'cc-by-nc-sa':
        attribute = 'alt="Creative Commons Attribution-NonCommercial-ShareAlike License"></a>';
        break;
      case 'cc-by-nc-nd':
        attribute = 'alt="Creative Commons Attribution-NonCommercial-NoDerivs License"></a>';
        break;
    }
    url = '<a href="https://creativecommons.org/licenses/' + code + '/4.0/">';
    attribute = '<img src="https://licensebuttons.net/l/' + code + '/4.0/88x31.png" ' + attribute;
  } else {
    attribute = '>All Rights Reserved</a>';
    url = '<a href="https://choosealicense.com/no-permission/"';
  }
  finalURL = url + attribute;
  return finalURL;
}
...
function countTrue(...values) {
  ...
  for (var i = 0; i < values.length; i++) {
    var value = values[i];
    if (value > 0) count_true++;
    else if (typeof value === 'boolean' && value === true) count_true++;
...
function countFalse(...values) {
  ...
  for (var i = 0; i < values.length; i++) {
    var value = values[i];
    if (typeof value === 'number' && value <= 0) count_false++;
    else if (typeof value === 'boolean' && value === false) count_false++;
    else if (typeof value === 'string') {
      value = value.toLowerCase();
      switch (value) {
        case 'no':
...
function buildQueryString(queryTerm, sortOrder, count) {
  queryTerm = encodeURIComponent(queryTerm);
  var flag = 0;
  if (count >= 10 && count <= 200 && (sortOrder === 'ascending' || sortOrder === 'descending'))
    flag = 1;
  if (flag === 1) {
    var query = '?query_term=' + queryTerm;
    var countTerm = '&count=' + count;
    var sortTerm = '';
    if (sortOrder === 'descending') sortTerm = '&' + sortOrder;
    var finalString = query + countTerm + sortTerm;
    return finalString;
  }
